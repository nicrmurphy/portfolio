...
const Header: Component<{ content: string }> = (props) => {
  return (
    // @ts-ignore
    <For each={props.content}>{(s, i) => <span>{s === " " ? "\xa0" : s}</span>}</For>
  );
};
...
const App: Component = () => {
  ...
  return (
    <div class={`${styles.App} ${styles.SlideFromLeft} ${styles.Background} ${slideOut() ? styles.SlideOut : ""}`}>
      <div
        class={`${styles.HomeContainer} ${styles.Foreground}`}
        onMouseEnter={() => setDisplayCheckerText(true)}
        onMouseLeave={() => setDisplayCheckerText(false)}
      >
        <div class={`${styles.HeaderText} ${displayCheckerText() ? fonts.CheckerText : ""}`}>
          <span style={{ color: "white" }}>Choose a&nbsp;</span>
          <Header content={"Game"}></Header>
        </div>
        <div class={styles.GameSelectionWrapper}>
          <div
            class={styles.GamePreview}
            onClick={() => onClick("/chess")}
            onMouseOver={() => onHover("chess")}
