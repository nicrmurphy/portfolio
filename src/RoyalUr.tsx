...
const RoyalUr: Component<{ BOARD_SIZE_PX: number, previewOnly?: boolean, highlightMoves?: boolean, highlightThreats?: boolean, resetBoard?: boolean }> = (props) => {  
  ...
  return (
    <>
    <div class={`${styles.BoardWrapper} ${previewOnly ? styles.PreviewBoard : ''} ${hoverFriendlyPiece() ? styles.Pointer : ''} ${displayPromotionDialog() ? styles.GreyBoard : ''}`}>
      <svg height={BOARD_SIZE_PX} width={BOARD_SIZE_PX} viewBox={`0 0 ${8 * TW} ${8 * TW}`} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} oncontextmenu={(e) => e.preventDefault()}>
        {/* Chessboard alternating color pattern */}
        <rect width="100%" height="100%" fill={'transparent'} />
        <For each={[0, 1, 2, 3, 4, 5, 6, 7]}>{(y) => 
          <For each={[0, 1, 2]}>{(x) => <>
            {[0, 1, 2, 3, 4, 5, 6, 7].includes(x) && !(y == 4 && x === 0) && !(y == 5 && x === 0) && !(y == 4 && x === 2) && !(y == 5 && x === 2) && <rect fill={lightSquareFill()} x={`${TW * (x)}`} y={`${TW * y}`} width={`${TW}`} height={`${TW}`} />}
            {props.highlightMoves && highlightedSquares()[getBoardIndexFromRankFile(y, x)] && <rect fill="blue" opacity=".5" x={`${TW * x}`} y={`${TW * y}`} width={`${TW}`} height={`${TW}`} />}
