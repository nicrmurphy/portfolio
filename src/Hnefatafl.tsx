...
const Chess: Component<{ BOARD_SIZE_PX: number, previewOnly: boolean }> = ({ BOARD_SIZE_PX, previewOnly }) => {  
  ...
  // const [lightSquareFill, setLightSquareFill] = createSignal<string>('#f0e2d5')
  // const [darkSquareFill, setDarkSquareFill] = createSignal<string>('#eed5ba')
  // const [kingSquareFill, setKingSquareFill] = createSignal<string>('darkblue')
  const [kingSquareFill, setKingSquareFill] = createSignal<string>('white')
  // const [kingSquareFill, setKingSquareFill] = createSignal<string>('#b58863')
  ...
  const movePiece = (prevIndex: number, newIndex: number, piece?: number): void => {
    ...
    if (ORTHOGONAL_OFFSETS.every(offset => NUM_SQUARES_TO_EDGE[getKingLocation()][offset] && (isEnemy(newBoard, getKingLocation(), getKingLocation() + offset) ||
      getKingLocation() + offset === throneIndex()))) setWinner('Attackers win!')

    // no more legal moves
    // console.log(moves)
    if (!moves[colorToMove()].filter(moves => moves?.length).length) setWinner(`${colorToMove() === Piece.White ? 'Attackers' : 'Defenders'} win!`)
    
    // king reaches corner
    if (kingSquares()[getKingLocation()] && getKingLocation() !== 60) setWinner('Defenders win!')
  ...
  const onMouseUp = (event: MouseEvent) => {
    ...
    setCursorStyle(getPieceColor(index) === colorToMove() ? 'Grab' : 'Default')
  ...
  const resetBoard = () => {
    setBoard(() => {
      const b = Array(NUM_BOARD_SQUARES).fill(0)
      // attacking pieces
